...
//println(list[1].length(), t list[2].length());
}
void draw() {
  noLoop();
}
...
int numberOfVowels(String sentence) {
  ...
  for (int q = 0; q < vowels.length; q++) {
    if (vowels[q] == vowels || vowels[q] == 'e' || vowels[q] == 'i' || vowels[q] == 'o' || vowels[q] == 'u' || vowels[q] == 'A' || vowels[q] == 'E' || vowels[q] == 'I' || vowels[q] == 'O' || vowels[q] == 'U') {
      amountOfVowels += 1;
    }
  }
